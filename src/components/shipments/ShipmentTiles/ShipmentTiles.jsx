...
const ShipmentTiles = ({ shipments, onSelectShipment, onEdit, onDelete, onFlag, userRole }) => {
      ...
      {shipments.map((shipment) => (
        ...
        >
          <div className={styles.header}>
            <div className={styles.shipmentNumber}>{shipment.shipmentNumber}</div>
            <div className={styles.menuWrapper}>
              ...
              {openMenu === shipment.id && (
                <div className={styles.menu} onClick={(e) => e.stopPropagation()}>
                  <button onClick={() => { onEdit(shipment); setOpenMenu(null); }}>
                    ✏️ Edit
                  </button>
                  <button onClick={() => { onFlag(shipment.id, !shipment.flagged); setOpenMenu(null); }}>
                    🚩 {shipment.flagged ? 'Unflag' : 'Flag'}
                  </button>
                  {userRole === 'ADMIN' && (
                    <button 
                      className={styles.delete}
                      onClick={() => { onDelete(shipment.id); setOpenMenu(null); }}
                    >
                      🗑️ Delete
                    </button>
                  )}
          ...
          <div className={styles.content}>
            ...
            <div className={styles.field}>
              ...
              <span className={styles.value}>{shipment.trackingNumber || 'N/A'}</span>
            </div>
          </div>
          
          <div className={styles.footer}>
            <div className={styles.info}>
              <span className={styles.rate}>${shipment.rate?.toFixed(2) || '0.00'}</span>
              <span className={styles.weight}>{shipment.weight ? `${shipment.weight} kg` : ''}</span>
            </div>
          </div>
        </div>
      ))}
